...
plt.savefig(f'PNG/Correlation_Matrix.png')  # 保存相关系数图
plt.show()
...
for column in numeric_data.columns:
    ...
    plt.savefig(f'PNG/Histogram_of__{column}.png')  # 保存直方图
    plt.close()
    ...
    plt.xlabel('Values')
    plt.savefig(f'PNG/Box_Plot_of__{column}.png')  # 保存箱线图
    plt.close()
    ...
    plt.xlabel('Values')
    plt.ylabel('Density')
    plt.savefig(f'PNG/Density_Plot_of__{column}.png')  # 保存密度图
    plt.close()
...
for column in data.columns:
    ...
    plt.savefig(f'PNG/Daily_Mean_Values_of__{column}.png')  # 保存折线图
    plt.show()
    ...
    plt.xticks(rotation=45)  # 旋转x轴刻度标签，以便更好地显示时间信息
    plt.savefig(f'PNG/Monthly_Mean_Values_of__{column}.png')  # 保存折线图
    plt.show()

# 调用PNG_merge.py脚本合并同种类型图像
subprocess.run(['python', 'PNG_merge.py'])
